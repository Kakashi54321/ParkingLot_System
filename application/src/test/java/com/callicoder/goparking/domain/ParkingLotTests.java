...
public class ParkingLotTests {
    ...
    public void leaveSlot_shouldClearSlot() {
        ParkingLot parkingLot = new ParkingLot(4); //A:4 O:0
        Car car = new Car("AN82CH1235", "Green");
        Ticket ticket1 = parkingLot.reserveSlot(car);//A:3 O:1
        Car car2 = new Car("GJ82CH1235", "Black");
        Ticket ticket2 = parkingLot.reserveSlot(car2);//A:2 O:2

        int numAvailableSlots = parkingLot.getAvailableSlots().size(); //2
        int numOccupiedSlots = parkingLot.getOccupiedSlots().size();//2

        ParkingSlot slot = parkingLot.leaveSlot(ticket1.getSlotNumber()); //A:3 O:1
